...
// data
import { Tea as EN } from "menu-en";
import { Tea as PT } from "menu-pt";
import { Tea as ES } from "menu-es";
import { Tea as FR } from "menu-fr";
      ...
      <div className="p-4">
        ...
        <div className="mb-10 mt-5 grid grid-cols-1 gap-x-3 gap-y-3 md:grid-cols-3">
          {menu?.map((item: MenuItem) => (
            <ItemCard key={item.id} name={item.name} description={item.description} price={item.price} options={item.options} />
          ))}
        </div>
